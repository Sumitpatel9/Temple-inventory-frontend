...
export default function MobileLowStockAlert() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.post(Api.lowStockAlert, body, {
        ...
        products.map((p, i) => (
          <div key={i} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100">
            ...
            <div className="flex gap-4 mt-2 flex-wrap">
              ...
              <span className={`text-xs font-semibold ${p.currentStock < p.minQty ? "text-red-600" : "text-gray-700"}`}>
                Stock: {p.currentStock} {p.uom} {p.currentStock < p.minQty ? "⚠ Low" : ""}
              </span>
            </div>
          </div>
        ))
